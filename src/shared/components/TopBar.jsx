export default function TopBar({
  ...
  // L31: drop falsy cities so an undefined city isn't counted in the ratio.
  const cities = [...new Set(stays.map((it) => it.city).filter(Boolean))];
  const booked = cities.filter((city) =>
    stays.some((it) => it.city === city && it.status === "conf"),
  ).length;
  ...
  // L31: guard undefined stop names → no "undefined to undefined".
  const first = stops?.[0]?.name;
  const last = stops?.[stops.length - 1]?.name;
  const tripName = first
    ? last && last !== first
      ? `${first} to ${last}`
      : first
    : "Trip";
      ...
      <div className="topbar-info">
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          >
            <title>Anisita</title>
            <path
              d="M6.5 4 C6.5 12, 15 20, 15 20 L23 20 L27 20 L31 20 L40 20 L46.5 20 L61 20"
              stroke="#7C3AED"
              strokeWidth="0.5"
        ...
        <div className="topbar-sub">
          ...
          <div className="topbar-bar" role="progressbar" aria-valuenow={pct}>
