...
export default function App() {
  ...
  // M51: trip data is scoped by email — don't silently scope to '' if it's missing.
  const email = session.user?.email;
  if (!email) {
    return (
      <div className="loading-screen">
        This account has no email address. Please sign in with an email-based
        account.
      </div>
    );
  }
  return (
    <TripProvider email={email}>
  ...
  return (
    <div className="app-shell">
      ...
      <TopBar
        ...
        stops={stops}
        session={session}
        onProfileClick={() => navigateTab("profile")}
        onRefresh={retryAll}
      />
      {!online && (
      ...
      <div className="page-container" role="main" id="main-content">
        {activeTab === "plan" && (
          <SelectPage
            filterCity={filterCity}
            clearFilterCity={() => setFilterCity(null)}
          />
        )}
      ...
      {!isProfile && (
        <>
          ...
          <button
            className="fab"
            onClick={() => setShowFab(showFab ? null : "menu")}
            aria-label="Add"
          >
            +
          </button>
      ...
      {showAddStop && (
        <AddStopModal onAdd={addStop} onClose={() => setShowAddStop(false)} />
      )}
      {showAddItem && (
        <AddItemModal
      ...
      {!isProfile && (
        <BottomTabs activeTab={activeTab} setActiveTab={navigateTab} />
      )}
    </div>
  );
}
