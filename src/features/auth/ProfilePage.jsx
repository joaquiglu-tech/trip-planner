import { useState, useEffect } from "react";
import { signOut, updateDisplayName } from "../../services/auth";
import { useSettings } from "../../shared/hooks/useSettings";

export default function ProfilePage({ session, onBack }) {
  const { darkMode, setDarkMode } = useSettings();
  const [displayName, setDisplayName] = useState("");
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [saveError, setSaveError] = useState("");

  const email = session?.user?.email || "";
  const initial = (displayName || email || "?")[0].toUpperCase();

  useEffect(() => {
    const meta = session?.user?.user_metadata;
    if (meta?.display_name) setDisplayName(meta.display_name);
  }, [session]);

  async function handleSave() {
    setSaving(true);
    setSaveError("");
    const { error } = await updateDisplayName(displayName);
    setSaving(false);
    if (error) {
      // L39: surface the failure instead of silently doing nothing.
      setSaveError(error.message || "Could not save your name.");
      return;
    }
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  }

  async function handleSignOut() {
    await signOut();
  }

  return (
    <div id="page-profile" className="page active">
      {onBack && (
        <button className="profile-back-btn" onClick={onBack}>
          <span aria-hidden="true">&larr;</span> Back
        </button>
      )}
      <div className="card">
        <div className="card-hd">Profile</div>
        <div
          className="card-bd"
          style={{
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
            gap: 16,
            padding: 24,
          }}
        >
          <div className="profile-avatar-lg">{initial}</div>
          <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
            {email}
          </div>
        </div>
      </div>

      <div className="card">
        <div className="card-hd">Settings</div>
        <div className="card-bd">
          <div className="settings-row">
            <label
              htmlFor="display-name"
              style={{ fontSize: 13, fontWeight: 600 }}
            >
              Display Name
            </label>
            <input
              id="display-name"
              type="text"
              value={displayName}
              onChange={(e) => setDisplayName(e.target.value)}
              placeholder="Your name"
              className="settings-input"
            />
          </div>
          <div className="settings-row">
            <button
              onClick={handleSave}
              disabled={saving}
              className="m-btn primary"
              style={{ width: "100%", padding: 10 }}
            >
              {saving ? "Saving..." : saved ? "✓ Saved" : "Save Name"}
            </button>
          </div>
          {saveError && (
            <div
              className="settings-row"
              style={{ color: "var(--danger-light)", fontSize: 12 }}
            >
              {saveError}
            </div>
          )}
          <hr className="sep" />
          <div
            className="settings-row"
            style={{
              display: "flex",
              justifyContent: "space-between",
              alignItems: "center",
            }}
          >
            <span style={{ fontSize: 13, fontWeight: 600 }}>Dark Mode</span>
            <label className="toggle-switch">
              <input
                type="checkbox"
                checked={darkMode}
                onChange={() => setDarkMode(!darkMode)}
              />
              <span className="toggle-slider"></span>
            </label>
          </div>
          <hr className="sep" />
          <div className="settings-row">
            <button
              onClick={handleSignOut}
              className="m-btn secondary btn-signout"
            >
              Sign Out
            </button>
          </div>
        </div>
      </div>

      <div className="card">
        <div className="card-hd">About</div>
        <div
          className="card-bd"
          style={{ fontSize: 12, color: "var(--text-muted)" }}
        >
          <span style={{ fontSize: 15, fontWeight: 700 }}>Anisita</span>
          <br />
          Trip planner · Real-time sync
        </div>
      </div>
    </div>
  );
}
