...
export default function TodayPage() {
  ...
  const activeStops = useMemo(() => {
    ...
    if (view.type === "date")
      return stops.filter(
        (s) =>
          view.date >= toDateStr(s.start_date) &&
          view.date <= toDateStr(s.end_date),
      );
    return [];
  }, [view, stops]);
  ...
  return (
    <div className="page active">
      ...
      <div className="itin-mode-toggle">
        <button
          className={`fp ${selectorMode === "stops" ? "fp-active" : ""}`}
          onClick={() => setSelectorMode("stops")}
        >
          Stops
        </button>
        <button
          className={`fp ${selectorMode === "dates" ? "fp-active" : ""}`}
          onClick={() => setSelectorMode("dates")}
        >
          Dates
        </button>
      </div>
        ...
        {selectorMode === "stops"
          ...
          : calendarDates.map((cd) => {
              ...
              return (
                <button
                  key={cd.date}
                  data-active={isDateActive(cd.date) ? "true" : "false"}
                  className={`today-sel-pill today-sel-pill-stop ${isDateActive(cd.date) ? "active" : ""} ${cd.date === todayDateStr ? "is-today" : ""}`}
                  onClick={() =>
                    cd.stopIdx >= 0 && setView({ type: "date", date: cd.date })
                  }
                  style={{
                    borderLeftColor: "var(--accent)",
                    minWidth: isMulti ? 120 : undefined,
                  }}
                >
                  <span
                    className="pill-stop-name"
                    title={cd.title}
                    style={
                      isMulti
                        ? {
      ...
      {view === "overview" ? (
        ...
        activeStops.map((stop) => (
          <StopSection
            ...
            userEmail={email}
            stops={stops}
            showTitle={activeStops.length > 1}
