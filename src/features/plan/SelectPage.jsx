...
export default function SelectPage({ filterCity, clearFilterCity }) {
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      if (sortField === "name")
        return (a.name || "").localeCompare(b.name || "") * dir; // M30: null-safe
      if (sortField === "price")
        return (
      ...
      if (sortField === "date")
        return (a.start_time || "zz").localeCompare(b.start_time || "zz") * dir;
      if (sortField === "status") {
  ...
  const sections = useMemo(() => {
    ...
    if (sortField === "status") {
      ...
      return STATUS_ORDER.filter((s) => byStatus[s]?.length).map((s) => ({
        key: s,
        label: `${STATUS_LABEL[s]} (${byStatus[s].length})`,
        items: byStatus[s],
      }));
    }
    ...
    if (sortField === "name" || sortField === "price" || sortField === "date") {
      return [
        { key: "all", label: `All items (${sorted.length})`, items: sorted },
      ];
    }
    ...
    return TYPE_ORDER.filter((k) => byType[k]?.length).map((k) => ({
      key: k,
      label: (TYPE_LABEL[k] || k) + " (" + byType[k].length + ")",
      items: byType[k],
    }));
